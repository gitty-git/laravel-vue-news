<template>
    <div>
        <inertia-link class="uppercase block my-2 font-sans font-bold" v-if="user" :href="'/admin'"><< Back</inertia-link>
        <div class="flex pt-4 items-center justify-between">
            ...
            <div class="flex">
                <inertia-link class="border border-gray-400 font-bold text-xs px-4 uppercase py-2 mx-2 rounded-full" :href="`/admin/unset-redactor/${user.id}`" v-if="user.roles.find(a => a.role === 'redactor')">
                    Unset as a redactor
                </inertia-link>

                <inertia-link class="border border-gray-400 font-bold text-xs px-4 uppercase py-2 mx-2 rounded-full" :href="`/admin/set-redactor/${user.id}`" v-else-if="user.roles.find(a => a.role !== 'redactor')">
                    Set as a redactor
                </inertia-link>

                <inertia-link class="border border-gray-400 font-bold text-xs px-4 uppercase py-2 mx-2 rounded-full" :href="`/admin/unset-admin/${user.id}`" v-if="user.roles.find(a => a.role === 'admin')">
                    Unset as an admin
                </inertia-link>

                <inertia-link class="border border-gray-400 font-bold text-xs px-4 uppercase py-2 mx-2 rounded-full" :href="`/admin/set-admin/${user.id}`" v-else-if="user.roles.find(a => a.role !== 'admin')">
                    Set as an admin
                </inertia-link>

                <inertia-link method="delete" :href="`/admin/delete-user/${user.id}`" class="bg-red-700 text-white uppercase font-bold text-xs px-4 py-2 mx-2 rounded-full">Delete user</inertia-link>
            </div>
        </div>
